...
> R:es <= MAX_ITEMS pero deberia ser < MAX_ITEMS
...
> R:R: Sí. La línea de carrito_descuento aparece con ##### porque los tests unitarios no llaman a esa función. La función se prueba recién en el test de integración.
...
> R:Un test unitario prueba una funcion en particular y uno de integracion prueba un sistema de funciones. El primero en detectar el error de carrito_total fue test_total_con_cantidad, uno unitario.
...
> R: Porque en C el buffer overflow puede sobreescribir memoria perteneciente a otras partes del sistema. En java y python no sucede porque controlan que la escritura este dentro de los limites antes de escribir
...
> R:Que los test se pueden repetir muchas veces a lo largo del proceso de desarrollo y da resultados mas objetivos.
...
> R:Porque cuando la cantidad es 1 da el mismo precio: 350 * 1 = 350. Pero al multiplicarse por 2, la cantidad, ahi muestra el fallo
```
